const SESSION_ID_KEY = "ek_chandeleur_session_id";
...
function renderResults(finalProfile) {
  ...
  const trackingData = JSON.stringify({
    ...
    profil: finalProfile,
    enseigne: window.APP_CONFIG?.ENSEIGNE || 'ambiance-styles'
  });
...
async function submitForm(e) {
  ...
  // Guard : si /shared/js/api.js n’est pas chargé => on ne peut pas appeler l’API
  if (!window.API || typeof window.API.createParticipant !== 'function') {
    showModal('⚠️', 'API non chargée', "Le fichier /shared/js/api.js n'est pas chargé (window.API undefined). Corrige tes <script> dans le HTML.");
    return;
  }
  ...
  // IMPORTANT : on fournit enseigne (contrainte BDD)
  const enseigne = (window.APP_CONFIG && window.APP_CONFIG.ENSEIGNE) ? window.APP_CONFIG.ENSEIGNE : "ambiance-styles";

  // Payload BDD (colle au schéma Postgres)
  const payload = {
    nom,
    prenom,
    email,
  ...
  try {
    ...
    console.error(err);
    showModal('⚠️', 'Envoi impossible', 'Une erreur est survenue lors de l’enregistrement. Merci de réessayer.');
  }
}
